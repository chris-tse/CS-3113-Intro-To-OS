...
main(int argc, char *argv[])
{
	...
	threads = (argc > 2) ? getInt(argv[2], GN_GT_0, "num-threads") : 2;
    ...
    /* Create num-threads number of threads that increment 'glob' */

	for (int i = 0; i < threads; i++) {
		s = pthread_create(&threadarr[i], NULL, threadFunc, &loops);
		if (s != 0)
	...
	for (int i = 0; i < threads; i++) {
		s = pthread_join(threadarr[i], NULL);
		if (s != 0)
